...
**표준 출력에 결과가 출력됨.**
- 미로를 구성하는 문자들에 대한 설명이 주어짐.
- 입력받은 미로를 1회 보여줌.
- DFS를 진행하며 도착하는 위치의 좌표들을 각각 (x, y) 형식으로 보여줌.
- DFS가 끝난 후 기록된 경로를 보여줌.
- BFS에 대해서도 DFS와 동일하게 보여줌.
...
## 결과 예시
- 20x20 미로. 출구를 찾는 경우와 찾지 못하는 경우를 비교해봄.
- 출구를 찾지 못하는 미로는 출구가 사방으로 고립되었다는 점 외에는 출구를 찾을 수 있는 미로와 구성이 같음.
- 왼쪽이 출구를 찾는 경우, 오른쪽이 출구를 찾지 못하는 경우.
- 탐색 방법은 DFS. 탐색 순서는 기본 옵션(상하좌우 순).
